    ...
    <main id="main-content">
      <section className="hero" aria-labelledby="hero-title">
        <div className="container hero-grid">
          <div className="hero-content">
            <p className="eyebrow eyebrow-line">{business.name.toUpperCase()} · {business.region.toUpperCase()}</p>
            <h1 id="hero-title"><span>Your water.</span><span>Starts right</span><span className="text-blue">here.</span></h1>
            <p className="hero-description">Professional water well drilling, pump installation and maintenance for residential and commercial properties across Northeast Florida.</p>
            <div className="hero-actions"><button className="button button-primary" onClick={() => openBooking()}>Request your free estimate<Icon name="arrow-up-right" size={19} /></button><a href={business.phoneHref} className="process-link"><span className="play-circle"><Icon name="phone" size={13} /></span>Or call {business.phoneDisplay}</a></div>
            <div className="hero-reassurance"><Icon name="check" size={13} />No cost. No obligation. Straight answers from the rig.</div>
            <div className="hero-trust"><div className="avatar-stack">{projects.slice(1, 4).map((project) => <Image key={project.image} src={project.image} width={36} height={36} alt="" />)}</div><div><div className="hero-rating"><Icon name="shield" size={13} /><span><strong>{stats[0].value} years</strong> drilling {business.region}</span></div><p>{stats[1].value} wells drilled · licensed &amp; insured in the State of Florida.</p></div></div>
          </div>
      ...
      <section className="trust-bar" aria-label="Aquifer Reach at a glance">
        <div className="container trust-stats">
          <div className="trust-stat"><strong>{stats[0].value.split("+")[0]}<span>+</span></strong><span>Years drilling<br />Northeast Florida</span></div>
          <div className="trust-stat"><strong>{stats[1].value}</strong><span>Wells drilled for homes,<br />farms and businesses</span></div>
          <div className="trust-stat"><strong>Free</strong><span>On-site estimate.<br />No obligation at all.</span></div>
          <div className="trust-stat"><span className="stat-shield"><Icon name="shield" size={33} /></span><span><b>Licensed &amp; insured.</b><br />Certified Florida contractor.</span></div>
        </div>
      </section>

      <section id="services" className="section services-section" aria-labelledby="services-title">
        <div className="container">
          <div className="section-heading"><div><p className="eyebrow eyebrow-line">WHAT WE OFFER</p><h2 id="services-title">Every property is different.<br />Our commitment isn’t.</h2></div><p>From a single home to a commercial site,<br className="desktop-break" /> we go deeper to find the right solution.</p></div>
          ...
          <p className="services-footnote"><Icon name="shield" size={16} />Licensed in Florida. Permits handled. Built to last.</p>
        </div>
      </section>
      ...
      <section id="why-aquifer" className="difference-section" aria-labelledby="difference-title">
        ...
        <div className="container difference-grid"><div className="difference-intro"><p className="eyebrow eyebrow-line">WHY CHOOSE US</p><h2 id="difference-title">Jacksonville’s most<br />trusted well<br /><span>drilling team.</span></h2><p>At {business.name}, we combine deep local knowledge of Florida’s aquifers with high-powered, tried-and-true equipment to tackle any drilling challenge. We’re proud to serve our fellow neighbours and Floridians across {business.region}.</p><ul className="proof-list">{proofPoints.map((point) => <li key={point}><Icon name="check" size={14} />{point}</li>)}</ul><div className="difference-signoff"><BrandMark /><span>More than a well.<br /><strong>A promise to do it right.</strong></span></div></div><div className="benefits">{promises.map((promise, index) => <div className="benefit" key={promise.title}><span className="benefit-icon"><Icon name={promise.icon} size={24} /></span><div><span className="benefit-number">0{index + 1} / OUR PROMISE</span><h3>{promise.title}</h3><p>{promise.description}</p></div></div>)}</div></div>
      </section>

      <section id="process" className="section process-section" aria-labelledby="process-title">
        <div className="container"><div className="center-heading"><p className="eyebrow">LESS GUESSWORK. MORE GOOD WATER.</p><h2 id="process-title">A simple start.<br />A well-informed decision.</h2><p>Big decisions feel smaller when you have the right people beside you.</p></div><div className="process-grid">{steps.map((step, index) => <article className="process-step" key={step.title}><div className="step-top"><span className="process-icon"><Icon name={step.icon} size={25} /></span><span className="step-line" /><span className="process-number">STEP 0{index + 1}</span></div><h3>{step.title}</h3><p>{step.description}</p></article>)}</div><div className="process-bottom"><button className="button button-primary" onClick={() => openBooking()}>Request my free estimate<Icon name="arrow-up-right" size={18} /></button><span><Icon name="clock" size={14} />Monday–Friday 8 am–6 pm · Saturday 8 am–12 pm</span></div></div>
      </section>

      <section id="work" className="reviews-section" aria-labelledby="work-title">
        <div className="container reviews-grid"><div className="reviews-intro"><p className="eyebrow eyebrow-line">OUR WORK</p><h2 id="work-title">Real jobs.<br />Florida ground.</h2><p>Rigs up, casing in, water running. A look at recent drilling and pump work across Jacksonville and the surrounding communities.</p><div className="reviews-score"><strong>{stats[1].value}</strong><div><span>Licensed &amp; insured</span><span>Family-owned since {business.established}</span></div></div></div><div className="work-grid">{projects.map((project) => <figure className="work-tile" key={project.image}><Image src={project.image} alt={project.alt} fill sizes="(max-width: 760px) 100vw, 60vw" /><figcaption>{project.caption}</figcaption></figure>)}</div></div>
      </section>

      <section id="faq" className="section faq-section" aria-labelledby="faq-title"><div className="container faq-grid"><div><p className="eyebrow eyebrow-line">CLEAR ANSWERS. RIGHT HERE.</p><h2 id="faq-title">A few good<br />questions.</h2><p className="faq-intro">New to wells? You’re in good company.<br />Let’s get beneath the surface.</p><button className="inline-link" onClick={() => openBooking()}>Something not covered here? Ask us with your request<Icon name="arrow-up-right" size={16} /></button></div><div className="faq-list">{faqs.map((faq, index) => <article className={`faq-item ${openFaq === index ? "is-open" : ""}`} key={faq.question}><h3><button id={`faq-question-${index}`} aria-expanded={openFaq === index} aria-controls={`faq-answer-${index}`} onClick={() => setOpenFaq(openFaq === index ? null : index)}>{faq.question}<span><Icon name={openFaq === index ? "minus" : "plus"} size={17} /></span></button></h3><div id={`faq-answer-${index}`} role="region" aria-labelledby={`faq-question-${index}`} hidden={openFaq !== index}><p className="faq-answer">{faq.answer}</p></div></article>)}</div></div></section>

      <section className="final-cta" aria-labelledby="cta-title"><div className="container"><div className="cta-panel"><Contours className="cta-contours" /><div><p className="eyebrow">SERVING {business.region.toUpperCase()} SINCE {business.established}</p><h2 id="cta-title">Good water is closer<br />than you think.</h2></div><div className="cta-action"><button className="button button-white" onClick={() => openBooking()}>Request my free estimate<Icon name="arrow-up-right" size={19} /></button><p><Icon name="check" size={13} />On-site estimate. Zero obligation. Clear answers.</p></div></div></div></section>
    </main>

    <footer className="site-footer"><div className="container"><div className="footer-grid"><div className="footer-brand"><Brand /><p>Deep local knowledge. Honest work.<br />Good water for generations to come.</p><span className="footer-made"><span />FLORIDA GROUND. FLORIDA GRIT.</span></div><div className="footer-column"><h3>EXPLORE</h3>{navigation.slice(0, 3).map((item) => <a key={item.href} href={item.href}>{item.label}</a>)}<a href="#work">Our work</a><a href="#faq">FAQs</a></div><div className="footer-column"><h3>WHAT WE DO</h3>{services.map((service) => <button key={service.id} onClick={() => setSelectedService(service)}>{service.title}</button>)}<button onClick={() => openBooking()}>Free on-site estimates<Icon name="arrow-up-right" size={13} /></button></div><div className="footer-column footer-contact"><h3>LET’S TALK WATER</h3><a className="footer-phone" href={business.phoneHref}>{business.phoneDisplay}<Icon name="arrow-up-right" size={16} /></a><p>{business.mailingAddress}<br /><small>{business.mailingNote}</small></p><p>{business.yardAddress}<br /><small>{business.yardNote}</small></p>{business.hours.map((hour) => <p key={hour.days} className="footer-hours"><span>{hour.days}</span> {hour.time}</p>)}<button className="footer-book" onClick={() => openBooking()}>Request an estimate<Icon name="arrow-right" size={16} /></button></div></div><div className="footer-bottom"><p>© {new Date().getFullYear()} {business.name}. All rights reserved.</p><div><button onClick={() => setLegal("privacy")}>Privacy policy</button><button onClick={() => setLegal("terms")}>Terms of use</button><a href="#top">Back to top<Icon name="arrow-up-right" size={13} /></a></div></div><p className="site-legal-note">{business.name} is a licensed and insured well drilling contractor in the State of Florida. Yard visits are by appointment only. Well depths, yields and water quality vary by property and are confirmed on site during your free estimate.</p></div></footer>
