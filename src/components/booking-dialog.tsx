...
export function BookingDialog({ initialService = "", onClose }: { initialService?: string; onClose: () => void }) {
  ...
  function downloadSummary() {
    ...
    const text = `AQUIFER REACH LLC — ESTIMATE REQUEST\n\nReference: ${reference}\nName: ${fields.fullName}\nEmail: ${fields.email}\nPhone: ${fields.phone}\nService: ${service}\nProperty ZIP: ${fields.zip}\nPreferred date: ${formatDate(fields.preferredDate)}\nTime preference: ${fields.timePreference}\nNotes: ${fields.notes || "None"}\n\nStatus: Request received — not a confirmed booking.\nNext step: a call from Aquifer Reach LLC on (904) 477-9809 to schedule the free on-site estimate.\n`;
  ...
  return <Modal onClose={onClose} labelledBy="booking-title" className="booking-modal">
    {reference ? <div className="success-view">
      ...
      <p>Thanks, {fields.fullName.trim().split(" ")[0]}. Your estimate request is with us — we’ll call you to arrange the on-site visit. Here’s your copy for the records.</p>
      ...
      <p className="form-disclaimer">A submitted request is not a confirmed booking. We’ll call you to lock in a date — Monday–Friday 8 am–6 pm, Saturday 8 am–12 pm.</p>
    </div> : <>
      <div className="booking-heading">
        ...
        <h2 id="booking-title">{step === 1 ? "Your free estimate starts here." : "How should we reach you?"}</h2>
      ...
      <form className="booking-form" onSubmit={submit} noValidate>
        ...
        {step === 1 ? <div className="form-grid">
          ...
          <div className="field field-full"><label htmlFor="booking-zip">Property ZIP code</label><input {...inputProps("zip")} value={fields.zip} onChange={(event) => update("zip", event.target.value.replace(/\D/g, "").slice(0, 5))} placeholder="e.g. 32256" autoComplete="postal-code" inputMode="numeric" maxLength={5} required />{fieldError("zip")}<p className="form-help">We’ll use this to confirm crew coverage in your area.</p></div>
          ...
          <div className="form-grid">
            ...
            <div className="field"><label htmlFor="booking-phone">Phone number</label><input {...inputProps("phone")} type="tel" value={fields.phone} onChange={(event) => update("phone", event.target.value)} placeholder="(904) 555-0142" autoComplete="tel" maxLength={32} required />{fieldError("phone")}</div>
            <div className="field field-full"><label htmlFor="booking-notes">Anything else we should know? <span className="optional">(optional)</span></label><textarea {...inputProps("notes")} value={fields.notes} onChange={(event) => update("notes", event.target.value)} placeholder="A new home, an acreage, a well that has lost pressure… Please tell us the address if you have one handy." maxLength={1500} rows={3} />{fieldError("notes")}</div>
            <div className="field-full"><label className="consent-label"><input {...inputProps("consent")} type="checkbox" checked={fields.consent} onChange={(event) => update("consent", event.target.checked)} required /><span>I agree to have these details stored so Aquifer Reach can respond to my estimate request. No marketing list, no obligation.</span></label>{fieldError("consent")}</div>
          </div>
        </>}
        ...
        <div className="form-actions">{step === 2 && <button className="back-button" type="button" onClick={() => setStep(1)} disabled={submitting}><Icon name="chevron-left" size={16} />Back</button>}<button className="button button-primary" type="submit" disabled={submitting}>{submitting ? <><span className="spinner" />Saving your request…</> : <>{step === 1 ? "Next: your details" : "Request my free estimate"}<Icon name="arrow-right" size={18} /></>}</button></div>
        ...
        <p className="form-disclaimer">We only use these details to prepare and quote your project. No marketing list, nothing sold on.</p>
      </form>
    </>}
  </Modal>;
}
